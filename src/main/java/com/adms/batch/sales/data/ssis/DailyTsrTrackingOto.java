...
//		test("D:/Work/Report/DailyReport/201501");
			...
			throws Exception
	{
		...
		fw.walk(sInputPath, new FilenameFilter()
		{
			...
			{
				return dir.getAbsolutePath().contains("OTO") && name.toUpperCase().contains("TSR") && name.toUpperCase().contains("TRACKING");
			}
		});
		...
		for (String fileName : fw.getFileList())
		{
			...
			new DailyTsrProductionFileTransform().transform("FileFormat_SSIS_DailyTsrTracking-input-OTO.xml", new File(fileName), "FileFormat_SSIS_DailyTsrTracking-output.xml", new File(outputFileNameSsis));
		}
	}
